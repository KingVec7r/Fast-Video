...
# ── Flash Attention 2 ──
# flash_attn_func handles much larger sequence lengths than torch's SDPA
# because it avoids materialising the N×N attention score matrix entirely,
# using tiled / online softmax to stay in SRAM.  This is critical when
# processing many video frames batched together in the vision encoder.
try:
...
class MHSA(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if _HAS_FLASH_ATTN:
            # Flash Attention 2: tiled / online-softmax attention that never
            # materialises the N×N score matrix.  This keeps the forward pass
            # safely within 32-bit CUDA indexing even for large B×N products.
            x = flash_attn_func(
                q, k, v,
            ...
            # Fallback: PyTorch native SDPA (also tries Flash Attention as a
            # backend, but may be less aggressive about large-sequence support).
            # q, k, v need to be [B, num_heads, N, head_dim] for SDPA.
            q_sdpa = q.transpose(1, 2)  # [B, num_heads, N, head_dim]
        ...
        # x: [B, N, num_heads, head_dim] → [B, N, C]
        x = x.reshape(B, N, C)
